...
ai_img_result = "" # 이미지 분석 후 결과를 담는 곳
...
@app.route('/AI/map', methods=['POST','GET'])
def AI_map():
    data = request.json
    longitude = data.get('longitude')
    latitude = data.get('latitude')
    # allergies = data.get('allergies')
    input_allergies = data.get('inputText')
    print('알레르기 정보 :' , input_allergies)
    
    if longitude is None or latitude is None:
        return jsonify({'message': '위치 정보가 없습니다.'}), 400

    # 위치 정보를 이용한 텍스트 생성
    input_text = f'위도 {latitude}도, 경도 {longitude}도 근처에 있는 {input_allergies}를 제외한 식당 이름과 메뉴를 알려줘.'
    ...
    try:
        ...
        ai_map_result = response.choices[0].message.content
        print(f'ai분석 :{ai_map_result}')
        return jsonify({'AI 분석결과': ai_map_result}), 200

    except KeyError as ke:
        # 응답 처리 오류
        print(f"Response parsing error: {ke}")
        print(f"Full response: {response}")  # 전체 응답 출력
        return jsonify({'message': '응답 처리 중 오류가 발생했습니다.'}), 500

    except Exception as e:
        # 기타 예외 처리
        print(f"Error during AI request: {e}")
        return jsonify({'message': 'AI 요청 중 오류가 발생했습니다.'}), 500
